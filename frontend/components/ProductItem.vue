<template>
  <div class="productItem" v-if="productItem">
    <div class="productItem__tag" v-if="productItem.price.old_price">
      Скидка
    </div>
    <img
      class="productItem__photo"
    ...
    <div class="productItem__info">
      ...
      <div class="productItem__infoOther">
        <div class="productItem__infoPrice">
          <div
        ...
        <div class="productItem__infoActions">
          <div
            class="productItem__infoAction productItem__infoAdd"
            v-if="!inBasket"
            @click="actionClick('toggleBasket', 'inBasket', 'basket')"
        >
            <svg
              width="36"
              height="36"
          ...
          <div
            class="productItem__infoAction productItem__infoBasket"
            v-else
            @click="actionClick('toggleBasket', 'inBasket', 'basket')"
          >
            <svg
          ...
          <div
            :class="[ 'productItem__infoAction productItem__infoLike', { 'active' : likeActive } ]"
            @click="actionClick('liked', 'likeActive', 'likedIds')"
        >
            <svg
              width="36"
              height="36"
            ...
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M22.0979 9.00001C20.795 9 19.9551 9.15522 18.9924 9.62647C18.6359 9.80094 18.3015 10.0111 17.991 10.2562C17.6924 10.0248 17.3713 9.82433 17.0296 9.65532C16.0482 9.1699 15.1652 9 13.9091 9C10.5448 9 8.1 11.7875 8.1 15.4079C8.1 18.142 9.62445 20.7831 12.464 23.3369C13.9544 24.6774 15.8574 26.004 17.2202 26.7097L18 27.1135L18.7798 26.7097C20.1426 26.004 22.0456 24.6774 23.536 23.3369C26.3755 20.7831 27.9 18.142 27.9 15.4079C27.9 11.8259 25.4322 9.01382 22.0979 9.00001ZM26.1 15.4079C26.1 17.5499 24.8276 19.7544 22.3324 21.9986C20.972 23.2221 19.2231 24.4449 18 25.0864C16.777 24.4449 15.0281 23.2221 13.6677 21.9986C11.1725 19.7544 9.90004 17.5499 9.90004 15.4079C9.90004 12.733 11.5953 10.8 13.9091 10.8C14.9079 10.8 15.5234 10.9185 16.2317 11.2688C16.65 11.4757 17.0198 11.749 17.3399 12.0904L18.0021 12.7969L18.6583 12.0848C18.9856 11.7297 19.3609 11.4502 19.7838 11.2432C20.4713 10.9067 21.0483 10.8 22.0942 10.8C24.3794 10.8095 26.1 12.7701 26.1 15.4079Z"
                fill="black"
              ></path>
            </svg>
          </div>
...
export default {
    ...
    mounted() {
        this.update('likeActive', 'likedIds');
        this.update('inBasket', 'basket');
    },
    methods: {
        priceFormating(price) {
            return Math.floor(price);
        },
        actionClick(mutationName, flagName, statusName) {
            this.$store.commit(mutationName, this.productItem.id);
            this.update(flagName, statusName);
        },
        update(flagName, statusName) {
            this[flagName] = this.$store.state[statusName].has(this.productItem.id);
        }
    }
};
</script>
